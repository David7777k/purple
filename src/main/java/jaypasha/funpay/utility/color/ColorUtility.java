...
public class ColorUtility {
    ...
    /**
     * Применяет прозрачность в процентах 0..100 к существующему ARGB.
     */
    ...
    /**
     * Применяет прозрачность, принимая opacity в 0..255.
     */
    ...
    /**
     * Применяет прозрачность с коэффициентом 0..1.
     */
    public static int applyOpacity01(int hex, float alpha01) {
        float p = Math.max(0f, Math.min(1f, alpha01));
        int originalAlpha = (hex >>> 24) & 0xFF;
        int newAlpha = Math.round(originalAlpha * p);
        return (newAlpha << 24) | (hex & 0x00FFFFFF);
    }
