...
public class KeybindsLayer extends DraggableLayer {
    ...
    static Supplier<List<ModuleLayer>> modules = () -> Pasxalka.getInstance().getModuleRepository().getModuleLayers().stream()
    ...
    public void render(DrawContext context, double mouseX, double mouseY, RenderTickCounter tickCounter) {
        ...
        modules.get().forEach(e -> {
            Api.text()
                    ...
                    .thickness(.1f)
                    .color(ColorUtility.applyOpacity(0xFFC9C3FF, (int) (50 * e.getAnimation().getOutput().floatValue())))
                    .build()
                    .render(context.getMatrices().peek().getPositionMatrix(), getX() - 5 + (7.5f * e.getAnimation().getOutput().floatValue()), getY() + 12.5f + offset.get());
            ...
            Api.text()
                    ...
                    .color(ColorUtility.applyOpacity(0xFFC9C3FF, (int) (50 * e.getAnimation().getOutput().floatValue())))
                    .build()
                    .render(context.getMatrices().peek().getPositionMatrix(), getX() + getWidth() + 5 - (7.5f * e.getAnimation().getOutput().floatValue()) - Api.inter().getWidth("[" + KeyBoardUtil.translate(e.getKey()) + "]", 5, 0.1f, 0), getY() + 12.5f + offset.get());
