...
public class StaffListLayer extends DraggableLayer {
    ...
    static List<String> staffsPrefixes = Lists.newArrayList("eternity", "dhelper", "d helper", "moder", "moderator", "admin", "owner", "administrator");

    static Supplier<List<AbstractClientPlayerEntity>> list = () -> mc.world.getPlayers().stream()
            .filter(player -> staffsPrefixes.stream().anyMatch(e -> {
                if (player.getScoreboardTeam() != null && player.getScoreboardTeam().getPrefix() != null) return player.getScoreboardTeam().getPrefix().getString().equalsIgnoreCase(e);
    ...
    public StaffListLayer() {
        super(10f, 95f, 60f, 15f, () -> module.get().getEnabled() && !list.get().isEmpty());
    }
    ...
    public void render(DrawContext context, double mouseX, double mouseY, RenderTickCounter tickCounter) {
        ...
        Api.text()
                ...
                .size(7)
                .build()
                .render(context.getMatrices().peek().getPositionMatrix(), getX() + 2.5f, getY() - 1 + (15 - Api.inter().getHeight("Keybinds", 7)) / 2);

        AtomicReference<Float> height = new AtomicReference<>(0f);
        list.get().forEach(e -> height.set(height.get() + 6));

        setHeight(15f + height.get());

        AtomicReference<Float> offset = new AtomicReference<>(0f);

        if (Pasxalka.getInstance().getModuleRepository().find(AttackAuraModule.class).getEnabled())
            mc.world.getPlayers().forEach(e -> {
                    System.out.println(e.getGameProfile().getName());
            });
        ...
        list.get().forEach(e -> {
            Api.text()
                    ...
                    .build()
                    .render(context.getMatrices().peek().getPositionMatrix(), getX() + 2.5f, getY() + 12.5f + offset.get());

            offset.set(offset.get() + 6);
        });
        Scissors.pop();
    }
}
