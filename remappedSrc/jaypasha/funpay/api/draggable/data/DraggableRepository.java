...
import jaypasha.funpay.api.draggable.data.impl.WaterMarkLayer;
...
public class DraggableRepository implements Api {
    ...
    Supplier<Boolean> altPreseed = () -> isKeyPressed(mc.getWindow().getHandle(), GLFW.GLFW_KEY_LEFT_ALT);
    Boolean netEnabled = true;

    public DraggableRepository() {
        list.add(new WaterMarkLayer());
    }
    ...
    public void update(DrawContext context, float tickDelta, int mouseX, int mouseY) {
        ...
        list.stream()
                ...
                .ifPresent(e -> {
                    ...
                    e.setX(altPreseed.get() ? e.getX() : netEnabled ? MathHelper.lerp(.1f, e.getX(), РезкаяpedX) : mouseX - (e.getWidth() / 2));
        ...
        list.forEach(e -> {
            ...
            if (jaypasha.funpay.utility.math.Math.isHover(mouseX, mouseY, e.getX(), e.getY(), e.getWidth(), e.getHeight())) {
                if (button == 0) {
                    if (!e.getDragging()) {
                        e.setDragging(true);
                        e.setPosition((float) (mouseX - e.getWidth() / 2), (float) (mouseY - e.getHeight() / 2));
                    }
                }
